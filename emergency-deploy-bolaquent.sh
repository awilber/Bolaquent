...
# Emergency deployment script for Bolaquent - SSM approach
# Based on CustomerSuccess successful deployment pattern

set -e

echo "🚨 EMERGENCY DEPLOYMENT: Bolaquent Vocabulary Learning App"
echo "Timestamp: $(date)"
...
curl -I http://$PUBLIC_IP/ 2>/dev/null | head -3 || echo "❌ Connection failed"

echo ""
echo "🛠️ Creating Bolaquent deployment script..."

# Create the Bolaquent deployment script
cat > bolaquent_emergency_fix.sh << 'BOLAQ_EOF'
#\!/bin/bash
...
echo "💾 Uploading Bolaquent deployment script to S3..."
aws s3 cp bolaquent_emergency_fix.sh s3://bolaquent-deployments/bolaquent_emergency_fix.sh

echo "🔧 Executing Bolaquent deployment via AWS SSM..."
echo "This will deploy the vocabulary learning app with pure black theme."

# Execute via SSM
if aws ssm send-command \
    --instance-ids $INSTANCE_ID \
    --document-name "AWS-RunShellScript" \
    --parameters 'commands=["cd /tmp","aws s3 cp s3://bolaquent-deployments/bolaquent_emergency_fix.sh ./bolaquent_emergency_fix.sh","chmod +x ./bolaquent_emergency_fix.sh","./bolaquent_emergency_fix.sh"]' \
    --output text > /tmp/ssm_bolaquent_command.out 2>&1; then
    
    echo "✅ SSM command sent successfully\!"
    echo "📋 Command output:"
    cat /tmp/ssm_bolaquent_command.out
    
    echo "⏳ Waiting 45 seconds for Bolaquent deployment..."
    sleep 45
    
    echo "🔍 Testing deployed Bolaquent app..."
    if curl -s http://$PUBLIC_IP/ | grep -q "Bolaquent"; then
        echo "🎉 SUCCESS\! Bolaquent Vocabulary Learning App is now live\!"
        echo "🎓 Pure black theme with ultra-compact spacing deployed\!"
    else
        echo "⚠️ Checking deployment status..."
        curl -I http://$PUBLIC_IP/ 2>/dev/null | head -3
...
echo ""
echo "🌐 Bolaquent Vocabulary Learning App URLs:"
echo "   Primary: http://$PUBLIC_IP/"
echo "   Direct:  http://$PUBLIC_IP:5000/"
echo "   Health:  http://$PUBLIC_IP:5000/health"
echo ""
echo "🎨 Features Deployed:"
echo "   ✅ Pure black theme (#000000 backgrounds)"
echo "   ✅ Ultra-compact spacing (1px-16px scale)"
echo "   ✅ Multi-tiered vocabulary learning (6 age tiers)"
echo "   ✅ 6,100+ vocabulary words"
echo "   ✅ AWS deployment via SSM (CustomerSuccess pattern)"
EOF < /dev/null