...
def practice():
    ...
    practice_words = db.session.query(VocabularyWord)\
        .outerjoin(UserProgress, 
                  (UserProgress.vocabulary_word_id == VocabularyWord.id) & 
                  (UserProgress.user_id == user.id))\
        .filter(VocabularyWord.tier_id == user.tier_id)\
        .filter(db.or_(
            UserProgress.mastery_level < 70,
            UserProgress.mastery_level.is_(None)
        ))\
        .limit(user.age_tier.words_per_session)\
        .all()
    
    return render_template('learning/practice.html', 
...
def achievements():
    ...
    return render_template('learning/achievements.html',
                         user=user,
